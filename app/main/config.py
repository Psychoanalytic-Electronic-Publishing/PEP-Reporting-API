...
class ConfigValues:
    def __init__(self, environment: str = None):
        ...
        with open("flask-config-overwrite.json", 'r') as f:
            ...
            if "environments" in overwrite:
                if "active" in overwrite and overwrite["active"] in overwrite["environments"]:
                    self.__overwrite_config_dict = overwrite["environments"].get(overwrite["active"])
                else:
                    #grab first entry if active not set
                    self.__overwrite_config_dict = next(iter(overwrite["environments"].values()))
            else:
        ...
        with open("flask-config.json", 'r') as f:
            self.__environment_config_dict = json.load(f)[self.requested_environment]

        #init values
        self.USE_ENCRYPTION = self.__get_config_value(Config.USE_ENCRYPTION, self.__string_to_bool) or False
        self.DEBUG = self.__get_config_value(Config.DEBUG, self.__string_to_bool) or False
        self.PRESERVE_CONTEXT_ON_EXCEPTION = self.__get_config_value(Config.PRESERVE_CONTEXT_ON_EXCEPTION, self.__string_to_bool) or False
        self.PROPAGATE_EXCEPTIONS = self.__get_config_value(Config.PROPAGATE_EXCEPTIONS, self.__string_to_bool) or False
        self.DATA_ERROR_TOPIC_ARN = self.__get_config_value(Config.DATA_ERROR_TOPIC_ARN) or ""
        self.DATA_ERROR_EMAIL_SUBJECT = self.__get_config_value(Config.DATA_ERROR_EMAIL_SUBJECT) or ""
        self.DYNAMODB_TABLE_DATA_ERROR = self.__get_config_value(Config.DYNAMODB_TABLE_DATA_ERROR) or ""
        ...
        boto3.setup_default_session(profile_name=self.AWS_PROFILE, region_name=self.AWS_REGION)
    ...
    def __get_config_value(self, key: str, typeConverterMethod=None, isEncrypted=False):
        ...
        #environment variable end up as strings so we allow to pass a type converter method to handle this
        ...
        if value is not None and isEncrypted is True and self.USE_ENCRYPTION is True:
            value = kms.decrypt(CiphertextBlob=bytes(base64.b64decode(value)))['Plaintext'].decode()
